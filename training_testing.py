...
def TrainTest_Alg2(train, test, target, drug_ids_list, X_columns, model_name, model_parameters):
    
    if "DRUG_ID" in train.columns:
        train.set_index("DRUG_ID", inplace=True)
    if "DRUG_ID" in test.columns:
    ...
    if model_name == "SVR":
        model = SVR(**model_parameters) 
    
    elif model_name== "KernelRidge" | model_name== "KR":
        model = KernelRidge(**model_parameters)
    else:
        print("Unknown model")
    ...
    for i, drug_id in enumerate(drug_ids_list):
        ...
        test_drug_true = test.loc[drug_id, target]
        test_drug_pred = test.loc[drug_id, "pred_"+target]
        mae[i] = mean_absolute_error(test_drug_true, test_drug_pred)
        mre[i] = mean_relative_error(test_drug_true, test_drug_pred)
    ...
    print("")
    print(train.shape, test.shape)
    return mean_mae, mean_mre, y_pred, train.shape, test.shape


def TrainTest_Alg1(train, test, target, drug_ids_list, model_name, X_columns, kernel, hyperparameters, 
                    degree = 3, print_drug_name = True):
    """hyperparameters in Algorithm 1 is a table with indexes as drug_ids and columns with corresponding 
    model parameters for each drug
    For the KernelRidge model, columns should be: alpha/gamma/ coef0/ degree 
    For the SVR model, columns should be: C/epsilon/coef0
    """
    mae = np.zeros(len(drug_ids_list))
    mre = np.zeros(len(drug_ids_list))
    ...
    for i, drug_id in list(enumerate(drug_ids_list)):
        ...
        if model_name == "SVR":
                model = SVR(kernel = kernel, **dict(hyperparameters.loc[drug_id, :]))
        elif (model_name== "KernelRidge") | (model_name== "KR"):
            model = KernelRidge(kernel = kernel, **dict(hyperparameters.loc[drug_id, :]))
        else:
            print("Unknown model")
