...
param_grid = dict(alpha = param_tested_alphas, solver = param_tested_solvers)

splitter_loo = LeaveOneOut()
grid = GridSearchCV(Ridge(), param_grid=param_grid, cv=splitter_loo, scoring = "neg_mean_absolute_error")
...
for i, data_set in list(enumerate(datasets)):
    ...
    model = Ridge(alpha=grid.best_params_["alpha"], solver = grid.best_params_["solver"])
    ...
    print("Dataset:", i, "best solver:", grid.best_params_["solver"])

results.to_csv(_FOLDER+"all_ridge_coef1.csv")
...
results.to_csv(_FOLDER+"all_ridge_coef2.csv")
...
results.to_csv(_FOLDER+"all_ridge_coef3.csv")
...
results.to_csv(_FOLDER+"all_ridge_coef4.csv")